...
class ImageDownloader:
    ...
    def _get_save_path(
        ...
        try:
            ...
            self.logger.warning(
                f'Unexpected date format: {date_str}, using fallback path'
            )
    ...
    def _download_image(
        ...
        try:
            ...
            current_ym = None
            if api_item:
                item_date_str = self._parse_bing_date(
                    api_item.get('date') or api_item.get('fullstartdate')
                )
                if item_date_str:
                    try:
                        item_date = datetime.strptime(item_date_str, '%Y-%m-%d')
                        current_ym = (item_date.year, item_date.month)
                    except ValueError:
                        pass
    ...
    def _fetch_api_images(self) -> List[Dict[str, Any]]:
        ...
        for attempt in range(1, max_retries + 1):
            self.logger.info(
                f'Fetching from Bing API (attempt {attempt}/{max_retries}): {url}'
            )
            try:
                ...
                self.logger.warning(
                    f'API fetch failed (attempt {attempt}/{max_retries}): {e}'
                )
                if attempt < max_retries:
                    ...
                    self.logger.error(
                        f'API fetch failed after {max_retries} attempts: {e}'
                    )
                    return []
    ...
    def _update_readme(
        ...
        for date_str, day_images in grouped.items():
            ...
            try:
                ...
                self.logger.info(
                    f'Updated README: {readme_path} ({total_count} entries)'
                )
            except Exception as e:
    ...
    def _update_json(
        ...
        for date_str, day_images in grouped.items():
            ...
            now = datetime.now()
            prev_minute = (now.minute // 10) * 10
            prev_time = now.replace(minute=prev_minute, second=0, microsecond=0)
            existing_data['updated_at'] = prev_time.strftime('%Y-%m-%dT%H:%M:%S.%f')
            existing_data['count'] = len(wallpapers)
            ...
            try:
                ...
                self.logger.info(
                    f'Updated JSON: {json_path} ({total} entries)'
                )
            except Exception as e:
...
class App:
    ...
    def _setup_signal_handlers(self) -> None:
        def signal_handler(signum: int, frame) -> None:
            self.logger.debug(f'Signal {signum} received, initiating shutdown...')
